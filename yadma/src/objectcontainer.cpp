...
yadmaptr<IWebServer> CObjectContainer::GetWebServer()
{
	if( !m_WebServer )
	{
		...
		//m_WebServer->AddListener( "/", yadmaptr<IWebServerListener>( new CIndexPage() ) );
...
{
	return m_DataBase ? m_DataBase : ( m_DataBase = yadmaptr<IDataBase>( new CDataBase( GetAppDirectory() + "../data/yadmadowns.dat" ) ) );	
}
